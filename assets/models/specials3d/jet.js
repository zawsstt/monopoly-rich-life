/* =====================================================================================
 * 大富翁 · 富贵人生 —— 载具 3D · 私人飞机（jet.js）
 * -------------------------------------------------------------------------------------
 * img2threejs 高保真复刻：refs/asset_private_jet.png（流线商务机 正前左 3/4 视图）
 * 规格工件：.img2threejs/spec_jet.json（--strict-quality PASS，第 2 轮）
 *
 *   window.Special3D.jet() → THREE.Group
 *   原点 = 机底中心（收起起落架的腹线 y=0，悬停姿态由 view3d.rideStart baseY 提供），
 *   机头朝 +Z，飞机自身左侧（左舷 PORT）为 +X。
 *   view3d 会整体放大（~2.5）并按行进方向旋转；userData.anim=[fn(t,dt)] 驱动航行灯。
 *   部件层级：全部 spec 组件为命名 Group（可点选/可拆解），网格 ~136 ≤ 240。
 *
 * 复刻要点（以参考图为准）：
 *   - 流线机身：14 站位分段旋成体（Lathe 16 棱面 + 纵向 0.95 卵形缩放），暖白漆面
 *   - 金色语言：金色鼻尖帽 / 风挡厚金框 / 舷窗金圈 / 全展金前缘（机翼+平尾）/ 金翼梢 /
 *     发动机金唇 + 金尾环 / 垂尾金顶盖
 *   - 驾驶舱风挡：4 块琥珀金玻璃（2 正前 + 2 侧窗）+ 中央金立柱，包裹式贴脊
 *   - 舷窗带：每侧 7 只八棱金圈 + 琥珀玻璃（12 段圆柱 ≥10）
 ...
 *   - 发动机：两具后机身两侧短舱（14 段圆柱），金进气唇 + 暗色径向风扇叶 + 锥形整流罩
 *     + 金尾环 + 暗尾塞 + 白 pylong + 外侧蓝条
 *   - 尾翼：后掠垂直尾翼（蓝前缘条双面 + 金顶盖）+ T 形平尾（全展金前缘 + 金翼尖）
...
var M = {
  ...
  steelK:  std('#383632', { rough: 0.45, metal: 0.6  }),            /* 尾塞暗钢 */
...
var HULL = [
  [-1.12, 0.105], [-1.06, 0.125], [-0.96, 0.155], [-0.82, 0.19], [-0.62, 0.235],
  [-0.35, 0.275], [0.00, 0.30], [0.35, 0.30], [0.62, 0.283], [0.85, 0.235],
  [1.02, 0.175], [1.14, 0.06]
];
function radiusAt(z) {
  var i, a, b;
...
function windowUnit(x, y, z) {
  ...
  if (!rimGeo) {
    ...
    rimGeo.rotateZ(PI / 2);
    lensGeo = new THREE.CylinderGeometry(0.028, 0.028, 0.02, 12);
...
/* 发动机短舱（s=+1 左舷 / -1 右舷）：短舱体+金唇+风扇+整流罩+金尾环+尾塞+吊架+蓝条。 */
var bladeGeo;
function engineUnit(s) {
  var g = grp('engine-unit');
  var nac = mesh(new THREE.CylinderGeometry(0.125, 0.105, 0.52, 14), M.paintW);
  nac.name = 'nacelle-body';
  nac.rotation.x = PI / 2;
  g.add(nac);
  var lip = mesh(new THREE.TorusGeometry(0.121, 0.026, 8, 16), M.gold);
  lip.name = 'intake-lip-gold';
  lip.position.z = 0.26;
  g.add(lip);
  var disc = mesh(new THREE.CylinderGeometry(0.096, 0.096, 0.02, 12), M.fanK);
  disc.name = 'fan-disc';
  disc.rotation.x = PI / 2;
  disc.position.z = 0.225;
  g.add(disc);
  if (!bladeGeo) bladeGeo = new THREE.BoxGeometry(0.012, 0.072, 0.006);
  var i, holder, blade;
  for (i = 0; i < 7; i++) {
    holder = grp('fan-blade-' + i);
    holder.rotation.z = (i / 7) * PI * 2;
    blade = mesh(bladeGeo, M.fanK);
    blade.position.y = 0.05;
    blade.rotation.x = 0.5;
    holder.add(blade);
    holder.position.z = 0.222;
    g.add(holder);
  }
  var spin = mesh(new THREE.ConeGeometry(0.026, 0.05, 10), M.fanK);
  spin.name = 'fan-spinner';
  spin.rotation.x = PI / 2;
  spin.position.z = 0.235;
  g.add(spin);
  var exR = mesh(new THREE.TorusGeometry(0.092, 0.013, 6, 14), M.gold);
  exR.name = 'exhaust-ring-gold';
  exR.position.z = -0.26;
  g.add(exR);
  var plug = mesh(new THREE.ConeGeometry(0.05, 0.1, 10), M.steelK);
...
window.Special3D.jet = function () {
  ...
  /* ======== fuselage：流线旋成体机身（14 站位 × 16 棱面，卵形 y0.95） ======== */
  var fuselage = grp('fuselage');
  var lathePts = [];
  for (i = 0; i < HULL.length; i++) {
    lathePts.push(new THREE.Vector2(HULL[i][1], HULL[i][0]));
  }
  var hullGeo = new THREE.LatheGeometry(lathePts, 16);
  ...
  var tplug = mesh(new THREE.ConeGeometry(0.055, 0.1, 10), M.steelK);
  ...
  /* ======== cockpit-canopy：4 块琥珀玻璃 + 厚金框（包裹风挡，骑在鼻脊上） ======== */
  ...
  /* ======== door：左前登机门（收起态=关闭，金框） ======== */
  var door = grp('door');
  var dSkin = box(0.016, 0.30, 0.185, M.paintW, 0, 0, 0);
  dSkin.name = 'door-skin';
  dSkin.position.set(flankX(0.36, 0.34, 0.985) + 0.002, 0.34, 0.36);
  door.add(dSkin);
  var dFrameA = box(0.014, 0.33, 0.02, M.gold, 0, 0, 0);
  dFrameA.name = 'door-frame-aft';
  dFrameA.position.set(flankX(0.26, 0.34, 0.985) + 0.004, 0.34, 0.262);
  door.add(dFrameA);
  var dFrameT = box(0.014, 0.02, 0.20, M.gold, 0, 0, 0);
  dFrameT.name = 'door-frame-top';
  dFrameT.position.set(flankX(0.36, 0.495, 0.985) + 0.004, 0.495, 0.36);
  door.add(dFrameT);
  var dFrameF = box(0.014, 0.33, 0.02, M.gold, 0, 0, 0);
  dFrameF.name = 'door-frame-fwd';
  dFrameF.position.set(flankX(0.46, 0.34, 0.985) + 0.004, 0.34, 0.458);
  door.add(dFrameF);
  g.add(door);

  /* ======== cabin-windows：每侧 7 只金圈琥珀窗（跨翼根均匀排布） ======== */
  ...
  for (s = -1; s <= 1; s += 2) {
    for (i = 0; i < zEdge.length - 1; i++) {
      ...
      var seg = bandSeg(dz + 0.015, 0.052, 0.022, M.blue, s, 0.315, zm, yaw, drop, 0, 'stripe-blue-seg');
      livery.add(seg);
      var yse = 0.278;
      var yaw2 = Math.atan((flankX(zEdge[i + 1], yse, 0.965) - flankX(zEdge[i], yse, 0.965)) / dz);
      var drop2 = (i === 0) ? 0.34 : 0;
      var pseg = bandSeg(dz + 0.015, 0.014, 0.02, M.gold, s, yse, zm, yaw2, drop2, 0, 'stripe-gold-pin');
      livery.add(pseg);
    }
  ...
  /* ======== wing-l / wing-r：低置后掠翼 + 全展金前缘 + 金翼梢（真镜像重建 Shape） ======== */
  ...
  /* ======== belly-fairing：翼身融合腹鳍（平滑椭球） ======== */
  var belly = grp('belly-fairing');
  var bfGeo = new THREE.SphereGeometry(1, 12, 8);
