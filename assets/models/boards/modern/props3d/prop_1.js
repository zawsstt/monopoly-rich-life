 ...
 * 同机位同角度——同一块地 40 年的生长史）。
 * 注册：window.Props3DModern[1](level 1..4) → THREE.Group（每次调用全新实例）
 * 契约：占地 ≤2.6×2.6；底面 y=0；正面 +z；每级 mesh ≤55；Canvas 纹理 ≤256px；
 *       源码零 Math.random（mulberry32 种子流）；动画 ≤2 项/模型（暖窗呼吸/霓虹闪烁）。
 *
 * 年代特征（对照参考图逐象限）：
 *   lv1 1990s 居民大院：灰砖合围院墙+双坡瓦顶房四合布局、纸木门窗、晾衣绳、
 *        门口杂物垛、院心大槐树、无商业痕迹
 *   lv2 2000s 临街首商：临街院墙打开、白灰墙面玻璃橱窗+蓝布雨棚、木牌幌、
 *        行道树+路灯、屋顶首批空调外机
 *   lv3 2010s 文创改造：整排玻璃门脸+连锁招牌、后房加高变屋顶露台（金属栏杆）+
 *        玻璃阳光房、墙面管线、花箱绿植成排
 *   lv4 2020s 全新运营：中堂钢结构玻璃披屋（分格幕墙）、东厢屋顶露台
 *        （木平台+玻璃栏板+廊架）、极简黑框通高幕墙+暖光内景、霓虹字号、外摆伞座
 *
 * 工程要点：手写按材质分桶合并 BufferGeometry（r147 无 BufferGeometryUtils），
 * 静态件全部合并成每材质 1 mesh；山墙屋顶用自建棱柱几何（UV 按尺寸烘焙保证贴图密度）。
 * ==================================================================================== */
(function () {
'use strict';
...
function mats() {
  ...
  MATS = {
    ...
    blue:     M('#3f6a8f', { map: texWeave(), rough: 0.8 })
  };
  return MATS;
}
...
/* 双坡棱柱（屋脊沿 x，y=0 为檐口高度，坡底宽 d）；UV：u 沿屋脊 w、v 沿坡长 */
var _prism = {};
function prismGeo(w, d, h, sc) {
  ...
  quad(D, Cc, F, E, w * sc, sl * sc);      /* 前坡 (+z) */
  quad(B, A, E, F, w * sc, sl * sc);       /* 后坡 (-z) */
...
/* 山墙封板三角（双面绕向，x=const 平面，宽 d 高 h） */
var _triG = {};
function triGeo(d, h) {
  var key = d + '_' + h;
...
/* ================= 5. 预制构件（各年代共用 → 同一块地的生长） ================= */
/* 沙盘：青石板广场 + 混凝土路缘 */
function buildPlinth(bag) {
  bag.box('pave', 2.5, 0.05, 2.5, 0, 0.025, 0, 0, 0, 0, SC.pave);
  bag.box('conc', 2.5, 0.03, 0.07, 0, 0.055, 1.235);
  bag.box('conc', 2.5, 0.03, 0.07, 0, 0.055, -1.235);
  bag.box('conc', 0.07, 0.03, 2.4, -1.235, 0.055, 0);
  bag.box('conc', 0.07, 0.03, 2.4, 1.235, 0.055, 0);
}
/* 灰砖院墙一段（含瓦檐墙帽；墙高低于建筑，衬托合院层次） */
function wallSeg(bag, x, z, len, alongX, h) {
  h = h || 0.24;
  if (alongX) {
    bag.box('brick', len, h, 0.06, x, 0.05 + h / 2, z, 0, 0, 0, SC.brick);
    bag.box('roofDk', len + 0.04, 0.035, 0.12, x, 0.05 + h + 0.018, z, 0, 0, 0, SC.tile);
  } else {
    bag.box('brick', 0.06, h, len, x, 0.05 + h / 2, z, 0, 0, 0, SC.brick);
    bag.box('roofDk', 0.12, 0.035, len + 0.04, x, 0.05 + h + 0.018, z, 0, 0, 0, SC.tile);
  }
}
/* 门柱 + 木门楼 */
function gatePiers(bag, x0, x1) {
  [x0, x1].forEach(function (x) {
    bag.box('brick', 0.1, 0.42, 0.14, x, 0.26, 1.06, 0, 0, 0, SC.brick);
    bag.box('roofDk', 0.16, 0.04, 0.2, x, 0.49, 1.06, 0, 0, 0, SC.tile);
  });
  bag.box('wood', 0.05, 0.34, 0.03, (x0 + x1) / 2 - 0.055, 0.22, 1.07, 0, 0, 0, SC.plank);
  bag.box('wood', 0.05, 0.34, 0.03, (x0 + x1) / 2 + 0.055, 0.22, 1.07, 0, 0, 0, SC.plank);
  bag.box('woodDk', 0.28, 0.04, 0.09, (x0 + x1) / 2, 0.435, 1.06);
}
/* 硬山顶坡屋（檐口 y=wallTop，含正脊/封山板） */
...
/* 纸木窗（1990s：木框 + 纸窗芯） */
function paperWin(bag, x, y, z, w, h, ry) {
  bag.box('woodDk', ry ? 0.03 : w, h, ry ? w : 0.03, x, y, z, 0, ry || 0, 0);
  bag.box('cream', (ry ? 0.033 : w - 0.05), h - 0.05, (ry ? w - 0.05 : 0.033), x, y, z, 0, ry || 0, 0);
}
/* 玻璃窗（木/钢框分格） */
...
function tree(bag, x, z, s) {
  bag.cyl('trunk', 0.03 * s, 0.045 * s, 0.42 * s, x, 0.05 + 0.21 * s, z, 0, 0, 0, 8);
  bag.sph('green', 0.24 * s, x, 0.05 + 0.52 * s, z, 12, 9);
  bag.sph('green2', 0.17 * s, x + 0.15 * s, 0.05 + 0.42 * s, z + 0.06 * s, 10, 8);
  bag.sph('green', 0.14 * s, x - 0.13 * s, 0.05 + 0.6 * s, z - 0.07 * s, 10, 8);
}
...
function lampPost(bag, x, z) {
  bag.cyl('frame', 0.016, 0.022, 0.62, x, 0.05 + 0.31, z, 0, 0, 0, 8);
  bag.box('frame', 0.16, 0.024, 0.024, x + 0.07, 0.66, z);
  bag.box('dark', 0.09, 0.035, 0.06, x + 0.15, 0.635, z);
  bag.sph('lampGlow', 0.024, x + 0.15, 0.61, z, 8, 6);
}
/* 花箱（木箱 + 绿球列） */
...
/* 木条箱 / 煤垛（年代杂物） */
function crate(bag, x, z, s, key) {
  s = s || 1;
  bag.box(key || 'wood', 0.14 * s, 0.1 * s, 0.11 * s, x, 0.05 + 0.05 * s, z, 0, 0.2 * (s - 1), 0, SC.plank);
}
...
/* 晾衣绳（两杆一线两衣——1990s 院子签名件） */
function clothesline(bag, x0, x1, z) {
  bag.cyl('woodDk', 0.014, 0.018, 0.5, x0, 0.3, z, 0, 0, 0, 6);
  bag.cyl('woodDk', 0.014, 0.018, 0.5, x1, 0.3, z, 0, 0, 0, 6);
...
function bench(bag, x, z, ry) {
  bag.box('wood', 0.4, 0.03, 0.13, x, 0.21, z, 0, ry || 0, 0, SC.plank);
  bag.box('woodDk', 0.03, 0.13, 0.11, x - 0.16, 0.115, z, 0, ry || 0, 0);
  bag.box('woodDk', 0.03, 0.13, 0.11, x + 0.16, 0.115, z, 0, ry || 0, 0);
}
/* 玻璃阳光房（屋面玻璃棱柱 + 分格钢肋） */
function sunroom(bag, x, y, z, w, d, rise, ry) {
  bag.prism('glass', w, d, rise, x, y, z, ry || 0, 0.5);
  bag.box('frame', (ry ? d : w) + 0.03, 0.02, 0.05, x, y + rise + 0.008, z, 0, ry || 0, 0);
  var n = 3, i;
  /* 竖肋：沿屋脊向两侧分格 */
  for (i = 0; i <= n; i++) {
    var px = x + (i / n - 0.5) * (ry ? d : w), pz = z;
    bag.box('frame', ry ? rise * 0.9 : 0.02, rise * 0.92, ry ? 0.02 : rise * 0.9, ry ? x : px, y + rise * 0.42, ry ? px : pz, 0, ry || 0, 0);
  }
}
/* 玻璃栏板（露台） */
function glassRail(bag, x, y, z, len, alongX) {
  bag.box('glass', alongX ? len : 0.015, 0.14, alongX ? 0.015 : len, x, y + 0.07, z);
  bag.box('frame', alongX ? len : 0.02, 0.018, alongX ? 0.02 : len, x, y + 0.15, z);
  var n = Math.max(2, Math.round(len / 0.3)), i;
  for (i = 0; i <= n; i++) {
    var t = -0.5 + i / n;
    bag.box('frame', alongX ? 0.018 : 0.018, 0.15, alongX ? 0.018 : 0.018, x + (alongX ? len * t : 0), y + 0.075, z + (alongX ? 0 : len * t));
  }
}
/* 木廊架（露台凉棚） */
function pergola(bag, x, y, z, w, d, h) {
  ...
  [[-w / 2 + 0.03, -d / 2 + 0.03], [w / 2 - 0.03, -d / 2 + 0.03], [-w / 2 + 0.03, d / 2 - 0.03], [w / 2 - 0.03, d / 2 - 0.03]].forEach(function (c) {
    bag.box('woodDk', 0.035, h, 0.035, x + c[0], y + h / 2, z + c[1]);
  });
  bag.box('woodDk', w, 0.03, 0.05, x, y + h, z - d / 2 + 0.03);
  bag.box('woodDk', w, 0.03, 0.05, x, y + h, z + d / 2 - 0.03);
  for (i = 0; i <= 4; i++) {
    bag.box('wood', 0.045, 0.02, d, x - w / 2 + (w * i) / 4, y + h + 0.022, z, 0, 0, 0, SC.plank);
  }
}
/* 遮阳伞座（外摆） */
function umbrella(bag, x, y, z, s) {
  s = s || 1;
  bag.cyl('frame', 0.012, 0.012, 0.34 * s, x, y + 0.17 * s, z, 0, 0, 0, 8);
  bag.cone('cream', 0.22 * s, 0.09 * s, x, y + 0.37 * s, z, 0, 0, 0, 10);
  bag.cyl('dark', 0.05, 0.06, 0.02, x, y + 0.01, z, 0, 0, 0, 10);
}
/* 圆桌 + 凳 */
function cafeTable(bag, x, z) {
  bag.cyl('metal', 0.09, 0.09, 0.015, x, 0.34, z, 0, 0, 0, 12);
  bag.cyl('frame', 0.012, 0.012, 0.28, x, 0.19, z, 0, 0, 0, 6);
  bag.cyl('dark', 0.07, 0.07, 0.012, x, 0.011, z, 0, 0, 0, 10);
  bag.cyl('metal', 0.055, 0.055, 0.012, x + 0.16, 0.2, z + 0.05, 0, 0, 0, 10);
  bag.cyl('frame', 0.01, 0.01, 0.19, x + 0.16, 0.1, z + 0.05, 0, 0, 0, 6);
}
/* 红灯笼（1990s-2000s 胡同签名件，静态） */
function lantern(bag, x, y, z, s) {
  s = s || 1;
  bag.sph('red', 0.055 * s, x, y - 0.05 * s, z, 10, 8);
  bag.cyl('metal', 0.022 * s, 0.03 * s, 0.016 * s, x, y + 0.006 * s, z, 0, 0, 0, 8);
  bag.cyl('metal', 0.018 * s, 0.024 * s, 0.014 * s, x, y - 0.1 * s, z, 0, 0, 0, 8);
  bag.cyl('red', 0.004 * s, 0.004 * s, 0.05 * s, x, y - 0.13 * s, z, 0, 0, 0, 6);
}

/* ================= 6. 四个年代（同布局同机位生长） =================
 * 固定语汇：2.5 见方青石板沙盘 + 灰砖院墙(±1.06) + 后正房(0.1,-0.68) + 东西厢(±0.85)
 *           + 临街倒座(±0.6,0.56) + 院心槐树(0,-0.02) + 南门楼(0,1.06)
 */
...
function stage1() {
  ...
  buildPlinth(bag);
  /* 合围院墙（南墙留门） */
  wallSeg(bag, 0, -1.06, 2.24, true);
  wallSeg(bag, -1.06, -0.06, 2.0, false);
  wallSeg(bag, 1.06, -0.06, 2.0, false);
  wallSeg(bag, -0.65, 1.06, 0.82, true);
  wallSeg(bag, 0.65, 1.06, 0.82, true);
  gatePiers(bag, -0.24, 0.24);
  /* 后正房：灰砖 + 纸木窗 + 木门 */
  bag.box('brick', 1.9, 0.42, 0.52, 0.1, 0.26, -0.68, 0, 0, 0, SC.brick);
  bag.tri('brick', 0.52, 0.16, 0.1 - 0.95, 0.47, -0.68, 0);
  bag.tri('brick', 0.52, 0.16, 0.1 + 0.95, 0.47, -0.68, 0);
  tileGable(bag, 0.1, 0.47, -0.68, 2.0, 0.62, 0.2, 0, true);
  paperWin(bag, -0.32, 0.28, -0.415, 0.22, 0.18);
  paperWin(bag, 0.52, 0.28, -0.415, 0.22, 0.18);
  bag.box('wood', 0.26, 0.34, 0.04, 0.1, 0.22, -0.415, 0, 0, 0, SC.plank);
  /* 东西厢 */
  [[-0.85], [0.85]].forEach(function (wx) {
    bag.box('brick', 0.42, 0.36, 1.06, wx[0], 0.23, -0.06, 0, 0, 0, SC.brick);
    bag.tri('brick', 0.5, 0.14, wx[0], 0.41, -0.06 - 0.58, PI / 2);
    bag.tri('brick', 0.5, 0.14, wx[0], 0.41, -0.06 + 0.52, PI / 2);
    tileGable(bag, wx[0], 0.41, -0.06, 1.16, 0.5, 0.16, PI / 2, true);
    paperWin(bag, wx[0], 0.26, 0.48, 0.18, 0.15);
  });
  /* 临街倒座（灰砖仓库相） */
  [[-0.6], [0.6]].forEach(function (fx) {
    bag.box('brick', 0.88, 0.36, 0.5, fx[0], 0.23, 0.56, 0, 0, 0, SC.brick);
    bag.tri('brick', 0.5, 0.14, fx[0] - 0.48, 0.41, 0.56, 0);
    bag.tri('brick', 0.5, 0.14, fx[0] + 0.48, 0.41, 0.56, 0);
    tileGable(bag, fx[0], 0.41, 0.56, 0.98, 0.6, 0.16, 0, true);
    bag.box('wood', 0.22, 0.26, 0.035, fx[0], 0.18, 0.815, 0, 0, 0, SC.plank);
    paperWin(bag, fx[0] + 0.26, 0.26, 0.815, 0.16, 0.14);
  });
  /* 院心槐树 + 晾衣绳 + 煤棚水缸 + 杂物 */
  tree(bag, 0, -0.08, 1.2);
  clothesline(bag, -0.45, 0.2, 0.22);
  bag.box('brick', 0.16, 0.26, 0.5, -0.94, 0.18, -0.75, 0, 0, 0, SC.brick);
  bag.prism('roofDk', 0.58, 0.24, 0.09, -0.94, 0.31, -0.75, PI / 2);
  bag.cyl('conc', 0.08, 0.065, 0.15, 0.42, 0.125, -0.34, 0, 0, 0, 10);
  bag.cyl('conc', 0.065, 0.055, 0.12, 0.6, 0.11, -0.3, 0, 0, 0, 10);
  crate(bag, 0.38, 0.82, 1);
  crate(bag, 0.4, 0.8, 0.7);
  brickPile(bag, -0.36, 0.84);
  shrub(bag, -0.98, 0.86, 0.07);
  shrub(bag, 1.0, -0.9, 0.06);
  /* 暖窗呼吸（唯一动画） */
  var glowM = glowMat(0.28);
  var gp = new THREE.Mesh(new THREE.PlaneGeometry(0.2, 0.16), glowM);
  gp.position.set(0.52, 0.28, -0.41); g.add(gp);
  g.userData.anim = [function (t) { glowM.emissiveIntensity = 0.2 + 0.14 * Math.sin(t * 1.4); }];
  bag.build(g);
...
function stage2() {
  ...
  wallSeg(bag, -1.06, -0.06, 2.0, false);
  wallSeg(bag, 1.06, -0.06, 2.0, false);
  wallSeg(bag, -1.0, 1.06, 0.32, true);
  wallSeg(bag, 1.0, 1.06, 0.32, true);
  gatePiers(bag, -0.24, 0.24);
  /* 后正房（同 lv1 骨架）+ 首台空调 */
  bag.box('brick', 1.9, 0.42, 0.52, 0.1, 0.26, -0.68, 0, 0, 0, SC.brick);
  bag.tri('brick', 0.52, 0.16, 0.1 - 0.95, 0.47, -0.68, 0);
  bag.tri('brick', 0.52, 0.16, 0.1 + 0.95, 0.47, -0.68, 0);
  tileGable(bag, 0.1, 0.47, -0.68, 2.0, 0.62, 0.2, 0, false);
  acUnit(bag, -0.4, 0.54, -0.72, PI / 2, 0);
  paperWin(bag, 0.52, 0.28, -0.415, 0.22, 0.18);
  bag.box('wood', 0.26, 0.34, 0.04, 0.1, 0.22, -0.415, 0, 0, 0, SC.plank);
  /* 东西厢（同骨架） */
  [[-0.85], [0.85]].forEach(function (wx) {
    ...
    bag.tri('brick', 0.5, 0.14, wx[0], 0.41, -0.06 - 0.58, PI / 2);
    bag.tri('brick', 0.5, 0.14, wx[0], 0.41, -0.06 + 0.52, PI / 2);
    tileGable(bag, wx[0], 0.41, -0.06, 1.16, 0.5, 0.16, PI / 2, false);
    paperWin(bag, wx[0], 0.26, 0.48, 0.18, 0.15);
  });
  /* 临街西倒座 → 首间商铺：白灰立面 + 玻璃橱窗 + 蓝布雨棚 */
  bag.box('brick', 0.88, 0.36, 0.5, -0.6, 0.23, 0.56, 0, 0, 0, SC.brick);
  bag.box('plaster', 0.9, 0.24, 0.03, -0.6, 0.3, 0.815, 0, 0, 0, SC.plaster);
  bag.tri('plaster', 0.5, 0.14, -1.08, 0.41, 0.56, 0);
  bag.tri('plaster', 0.5, 0.14, -0.12, 0.41, 0.56, 0);
  tileGable(bag, -0.6, 0.41, 0.56, 0.98, 0.6, 0.16, 0, false);
  glassWin(bag, -0.68, 0.27, 0.828, 0.42, 0.2, 0);
  bag.box('woodDk', 0.2, 0.26, 0.03, -0.36, 0.18, 0.83);
  awning(bag, 'blue', -0.68, 0.4, 0.845, 0.5, 0.18, 0.32);
  /* 东倒座保持灰砖 + 新木窗 */
  bag.box('brick', 0.88, 0.36, 0.5, 0.6, 0.23, 0.56, 0, 0, 0, SC.brick);
  bag.tri('brick', 0.5, 0.14, 0.12, 0.41, 0.56, 0);
  bag.tri('brick', 0.5, 0.14, 1.08, 0.41, 0.56, 0);
  tileGable(bag, 0.6, 0.41, 0.56, 0.98, 0.6, 0.16, 0, true);
  glassWin(bag, 0.6, 0.25, 0.815, 0.3, 0.16, 0);
  /* 街道家具：行道树 + 路灯 + 条凳；院心槐树延续 */
  tree(bag, 0, -0.08, 1.2);
  tree(bag, -0.95, 0.92, 1.05);
  tree(bag, 0.95, 0.9, 0.95);
  lampPost(bag, -0.02, 1.12);
  bench(bag, 0.42, 0.98, 0);
  shrub(bag, 0.98, -0.9, 0.06);
  /* 招牌：木牌幌「副食百货」 */
  var sg = signMesh('副食百货', 0.4, 0.09);
  sg.position.set(-0.6, 0.365, 0.835); g.add(sg);
  /* 店内暖光呼吸 */
  var glowM = glowMat(0.3);
  var gp = new THREE.Mesh(new THREE.PlaneGeometry(0.4, 0.18), glowM);
  gp.position.set(-0.68, 0.27, 0.82); g.add(gp);
  g.userData.anim = [function (t) { glowM.emissiveIntensity = 0.24 + 0.14 * Math.sin(t * 1.5); }];
  bag.build(g);
...
function stage3() {
  ...
  wallSeg(bag, 1.0, 1.06, 0.32, true);
  gatePiers(bag, -0.24, 0.24);
  /* 后正房加高 → 屋顶露台（金属栏杆）+ 玻璃阳光房 + 双空调 + 墙面管线 */
  bag.box('brick', 1.9, 0.52, 0.52, 0.1, 0.31, -0.68, 0, 0, 0, SC.brick);
  bag.box('conc', 1.94, 0.04, 0.56, 0.1, 0.585, -0.68);
  glassRail(bag, 0.1, 0.6, -0.41, 1.9, true);
  bag.box('plaster', 1.9, 0.2, 0.03, 0.1, 0.685, -0.43, 0, 0, 0, SC.plaster);
  glassWin(bag, -0.3, 0.685, -0.428, 0.3, 0.14, 0);
  glassWin(bag, 0.5, 0.685, -0.428, 0.3, 0.14, 0);
  sunroom(bag, 0.55, 0.6, -0.7, 0.7, 0.42, 0.15, 0);
  acUnit(bag, -0.62, 0.66, -0.72, PI / 2, 0.28);
  bag.cyl('metal', 0.009, 0.009, 0.5, 0.86, 0.42, -0.5, 0, 0, 0, 6);
  /* 东西厢 */
  [[-0.85], [0.85]].forEach(function (wx) {
    bag.box('brick', 0.42, 0.36, 1.06, wx[0], 0.23, -0.06, 0, 0, 0, SC.brick);
    bag.tri('brick', 0.5, 0.14, wx[0], 0.41, -0.06 - 0.58, PI / 2);
  ...
  /* 临街双铺 → 整排玻璃门脸（西：文创工社；东：咖啡） */
  [[-0.6, 'plaster', 'red', '文创工社'], [0.6, 'brick', 'dark', 'COFFEE']].forEach(function (cfg) {
    var fx = cfg[0];
    bag.box('brick', 0.88, 0.36, 0.5, fx, 0.23, 0.56, 0, 0, 0, SC.brick);
    if (cfg[1] === 'plaster') bag.box('plaster', 0.9, 0.26, 0.03, fx, 0.31, 0.815, 0, 0, 0, SC.plaster);
    bag.tri(cfg[1], 0.5, 0.14, fx - 0.48, 0.41, 0.56, 0);
    bag.tri(cfg[1], 0.5, 0.14, fx + 0.48, 0.41, 0.56, 0);
    tileGable(bag, fx, 0.41, 0.56, 0.98, 0.6, 0.16, 0, false);
    glassWin(bag, fx - 0.08, 0.26, 0.828, 0.5, 0.22, 0);
    bag.box('woodDk', 0.18, 0.26, 0.03, fx + 0.32, 0.18, 0.83);
    if (cfg[2] !== 'dark') awning(bag, cfg[2], fx - 0.08, 0.41, 0.845, 0.56, 0.18, 0.32);
  });
  /* 街道：花箱列 + 行道树 + 路灯；院心槐树延续 */
  tree(bag, 0, -0.08, 1.25);
  planterBox(bag, -0.55, 1.12, true);
  planterBox(bag, 0.55, 1.12, true);
  tree(bag, -0.95, 0.98, 1.05);
  tree(bag, 0.95, 0.98, 0.95);
  lampPost(bag, -0.02, 1.12);
  bench(bag, 0.42, 0.98, 0);
  /* 招牌 ×2：横匾 + 竖挂 */
  var sg1 = signMesh('文创工社', 0.4, 0.095);
  sg1.position.set(-0.6, 0.465, 0.835); g.add(sg1);
  var sg2 = signMesh('咖啡', 0.07, 0.24, { vertical: true });
  sg2.position.set(1.045, 0.3, 0.6); sg2.rotation.y = PI / 2; g.add(sg2);
  /* 店内暖光 */
  var glowM = glowMat(0.32);
  var gp1 = new THREE.Mesh(new THREE.PlaneGeometry(0.48, 0.2), glowM); gp1.position.set(-0.68, 0.26, 0.822); g.add(gp1);
  var gp2 = new THREE.Mesh(new THREE.PlaneGeometry(0.48, 0.2), glowM); gp2.position.set(0.52, 0.26, 0.822); g.add(gp2);
...
function stage4() {
  ...
  wallSeg(bag, 1.0, 1.06, 0.32, true);
  gatePiers(bag, -0.24, 0.24);
  /* 中堂 → 钢结构玻璃披屋（分格幕墙 + 玻璃两坡顶 + 避雷针） */
  bag.box('conc', 1.9, 0.06, 0.52, 0.1, 0.08, -0.68);
  bag.box('glass', 1.86, 0.44, 0.48, 0.1, 0.31, -0.68);
  var i;
  for (i = 0; i <= 6; i++) {
    var px = 0.1 - 0.93 + i * 0.31;
    bag.box('frame', 0.035, 0.44, 0.035, px, 0.31, -0.455);
    bag.box('frame', 0.035, 0.44, 0.035, px, 0.31, -0.905);
  }
  bag.box('frame', 1.9, 0.035, 0.035, 0.1, 0.515, -0.455);
  bag.prism('glass', 2.0, 0.62, 0.3, 0.1, 0.53, -0.68, 0, 0.5);
  bag.box('frame', 2.04, 0.025, 0.06, 0.1, 0.84, -0.68);
  for (i = 1; i < 6; i++) {
    bag.box('frame', 0.028, 0.3, 0.028, 0.1 - 1.0 + i * 0.4 * 0.99, 0.68, -0.455, 0, 0, 0);
    bag.box('frame', 0.028, 0.3, 0.028, 0.1 - 1.0 + i * 0.4 * 0.99, 0.68, -0.905, 0, 0, 0);
  }
  bag.cyl('metal', 0.006, 0.012, 0.2, 0.1, 0.95, -0.68, 0, 0, 0, 6);
  bag.sph('metal', 0.014, 0.1, 1.05, -0.68, 8, 6);
  /* 东厢 → 屋顶露台（木平台 + 玻璃栏板 + 廊架 + 伞座） */
  bag.box('brick', 0.42, 0.36, 1.06, 0.85, 0.23, -0.06, 0, 0, 0, SC.brick);
  bag.box('wood', 0.5, 0.035, 1.14, 0.85, 0.415, -0.06, 0, 0, 0, SC.plank);
  glassRail(bag, 0.85, 0.43, 0.49, 1.1, false);
  glassRail(bag, 0.85, 0.43, -0.61, 1.1, false);
  bag.tri('brick', 0.5, 0.14, 0.85, 0.41, -0.64, PI / 2);
  bag.tri('brick', 0.5, 0.14, 0.85, 0.41, 0.52, PI / 2);
  pergola(bag, 0.85, 0.43, -0.28, 0.44, 0.6, 0.24);
  umbrella(bag, 0.85, 0.45, 0.28, 0.8);
  acUnit(bag, 0.85, 0.5, -0.85, PI / 2, 0);
  /* 西厢（同骨架，加空调） */
  bag.box('brick', 0.42, 0.36, 1.06, -0.85, 0.23, -0.06, 0, 0, 0, SC.brick);
  bag.tri('brick', 0.5, 0.14, -0.85, 0.41, -0.64, PI / 2);
  bag.tri('brick', 0.5, 0.14, -0.85, 0.41, 0.52, PI / 2);
  tileGable(bag, -0.85, 0.41, -0.06, 1.16, 0.5, 0.16, PI / 2, false);
  acUnit(bag, -0.85, 0.52, -0.5, 0, 0.3);
  /* 临街双铺 → 极简黑框通高幕墙 */
  [[-0.6], [0.6]].forEach(function (cx) {
    var fx = cx[0];
    bag.box('brick', 0.88, 0.2, 0.5, fx, 0.15, 0.56, 0, 0, 0, SC.brick);
    bag.box('plaster', 0.9, 0.16, 0.03, fx, 0.33, 0.815, 0, 0, 0, SC.plaster);
    bag.tri('plaster', 0.5, 0.14, fx - 0.48, 0.41, 0.56, 0);
    bag.tri('plaster', 0.5, 0.14, fx + 0.48, 0.41, 0.56, 0);
    tileGable(bag, fx, 0.41, 0.56, 0.98, 0.6, 0.16, 0, false);
    bag.box('glassDk', 0.8, 0.32, 0.03, fx, 0.21, 0.828);
    for (i = 0; i <= 3; i++) {
      bag.box('dark', 0.03, 0.34, 0.045, fx - 0.4 + i * 0.267, 0.21, 0.832);
    }
    bag.box('dark', 0.84, 0.03, 0.05, fx, 0.385, 0.83);
  });
  /* 外摆：伞座 + 桌凳 + 花箱列 + 行道树；院心槐树延续 */
  tree(bag, 0, -0.08, 1.3);
  umbrella(bag, -0.45, 0.05, 1.0, 1);
  cafeTable(bag, -0.45, 1.0);
  umbrella(bag, 0.45, 0.05, 0.98, 1);
  cafeTable(bag, 0.45, 0.98);
  planterBox(bag, -0.7, 1.12, true);
  planterBox(bag, 0.7, 1.12, true);
  tree(bag, -0.95, 0.98, 1.05);
  tree(bag, 0.95, 0.98, 0.95);
  shrub(bag, 0.3, 0.92, 0.07);
  /* 门匾 + 霓虹字号 */
  var sg1 = signMesh('南锣鼓巷', 0.44, 0.1, { frameColor: '#d8c9a0' });
  sg1.position.set(0, 0.475, 1.1); g.add(sg1);
  var neon = signMesh('NLG·COFFEE', 0.42, 0.08, { bg: '#101317', frame: false, fg: '#ffd9a0', neon: true, neonColor: '#ffbe6e' });
  neon.position.set(0.6, 0.455, 0.835); g.add(neon);
  /* 暖光内景 + 霓虹闪烁（两项动画封顶） */
  var glowM = glowMat(0.34);
  var gp1 = new THREE.Mesh(new THREE.PlaneGeometry(1.7, 0.32), glowM); gp1.position.set(0.1, 0.29, -0.45); g.add(gp1);
  var gp2 = new THREE.Mesh(new THREE.PlaneGeometry(0.74, 0.26), glowM); gp2.position.set(-0.6, 0.21, 0.822); g.add(gp2);
