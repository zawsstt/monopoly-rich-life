/* 大富翁·现代写实棋盘（蓉）格 16「宽窄巷子」—— 成都灰砖宽/窄/井三巷 + 川西灰瓦门头 + 茶馆座，四年代演进 · v2 材质精修
 * 契约：window.Props3DModern[16](level 1..4) → Group，每次全新实例；占地 ≤2.6×2.6；底面 y=0；正面 +Z；每级 mesh ≤55（v2 增量 ≤+6，实际 -3）；
 ...
 *   材质分区：石板 0.92 / 青砖 0.88 / 灰泥 0.85 / 瓦 0.7 / 木 0.72 / 树干 0.9 / 玻璃 0.15·metal0.6 / 金饰 0.35·metal0.8 / 钢 0.45·metal0.7；灯笼纸纹与匾额 map+emissiveMap。
 * 年代特征（refs/modern/prop_16.png）：lv1 1990s 素面灰砖+小门头+木门板+稀树；lv2 2000s +匾额灯笼+茶馆白伞+披屋；lv3 2010s +双层门楼+暖光店面+灯笼串+街灯+加建；lv4 2020s +玻璃阳光房+满巷灯笼+花箱+茂树。 */
(function () {
'use strict';
...
function tex(id) { if (_TX[id]) return _TX[id]; var c = id === 'g' ? cv(256, 64) : cv(128, 128); TXF[id](c.getContext('2d'), c.width, c.height, lcg(1600 + id.charCodeAt(0))); var t = new THREE.CanvasTexture(c); t.encoding = THREE.sRGBEncoding; t.wrapS = t.wrapT = THREE.RepeatWrapping; t.anisotropy = 4; return (_TX[id] = t); }
...
function EAVE(m, w, h, t, tip, d, x, y, z) { /* 川西卷檐坡屋顶： quadratic 翘檐断面沿脊挤出 */
...
function build(level) {
  ...
  BK = {};
  var A = { /* 材质分区（桶）：石板/青砖/瓦/木/树干/玻璃/金/钢/布/线 —— 同桶不同顶点色（stone/lane/plaza、leaf/leaf2 合桶） */
    stone: MS('#7e766a', 0.92, 0, 's', 0.6), lane: MS('#6f685e', 0.92, 0, 's', 0.6), plaza: MS('#a49b8b', 0.92, 0, 's', 0.6),
    wall: MS(['#aa9e94', '#b4a79c', '#bcafa4', '#b9ada2'][level - 1], 0.88, 0, 'b', 0.42),
    ...
    leaf: MS('#6e7c48', 0.95, 0, 'l', 0.3), leaf2: MS('#82905a', 0.95, 0, 'l', 0.3), white: MS('#ddd6c6', 0.85, 0, 'p', 0.8),
    sky: MS('#354049', 0.12, 0.5, '', 0, '', 0, 0.55), steel: MS('#7d848a', 0.45, 0.7), umb: MS('#e8e0cc', 0.8), gold: MS('#c9a04a', 0.35, 0.8),
    wire: MS('#3d3b38', 0.8), awn: MS('#7c4a38', 0.85)
  };
  var glow = MS('#5a4a38', 0.2, 0, '', 0, '#ffbd66', 0.14); /* 暖光窗玻璃：低 rough */
  var lant = MS('#b03828', 0.6, 0, 'n', 0.12, '#ff5a2a', level >= 2 ? 0.3 : 0.05); /* 灯笼纸：map+emissiveMap（lv1 微光） */
  var signM = level >= 2 ? MS('#ffffff', 0.5, 0, 'g', 0, '#ffcf8e', 0.12) : null; /* 匾额：保留原 UV，map+emissiveMap */
  var lampM = level >= 3 ? MS('#ffe6b0', 0.4, 0, '', 0, '#ffd9a0', 0.7) : null;
  function lan(x, y, z, s, hl) { s = s || 1; if (hl) Y(A.wire, 0.004, 0.004, 0.07 * s, 4, x, y + 0.075 * s, z); Y(A.gold, 0.013 * s, 0.013 * s, 0.022 * s, 6, x, y + 0.04 * s, z); Y(lant, 0.033 * s, 0.04 * s, 0.058 * s, 6, x, y, z); Y(A.gold, 0.007 * s, 0.005 * s, 0.028 * s, 4, x, y - 0.043 * s, z); }
  /* 地面：石板坪 + 宽巷主道 + 窄巷横巷（三巷肌理） */
  B(A.stone, 2.6, 0.06, 1.9, 0, 0.03, 0); B(A.lane, 2.6, 0.064, 0.5, 0, 0.032, 0.1); B(A.lane, 0.16, 0.064, 0.76, -0.205, 0.032, -0.55);
  /* 后排：灰砖院落 ×3 + 川西灰瓦卷檐 */
  B(A.wall, 1.0, 0.5, 0.66, -0.78, 0.31, -0.56); EAVE(A.roof, 0.8, 0.15, 0.028, 0.05, 1.02, -0.78, 0.56, -0.56);
  B(A.wall, 0.94, 0.58, 0.66, 0.32, 0.35, -0.56); EAVE(A.roof, 0.8, 0.17, 0.028, 0.055, 0.96, 0.32, 0.64, -0.56);
  B(A.wall, 0.46, 0.46, 0.66, 1.05, 0.29, -0.56); EAVE(A.roof, 0.76, 0.13, 0.026, 0.045, 0.5, 1.05, 0.52, -0.56);
  var wx = [-1.04, -0.5, 0.1, 0.56], i;
  for (i = 0; i < 4; i++) B(i === 2 || level >= 3 ? glow : A.win, 0.14, 0.17, 0.02, wx[i], 0.32, -0.222);
  /* 前排商铺（面向 +Z）×2 + 右前石板广场（树/灯/座椅/花箱，参考图右下前院） */
  B(A.wall, 0.66, 0.4, 0.5, -0.31, 0.26, 0.69); EAVE(A.roof, 0.6, 0.11, 0.024, 0.04, 0.74, -0.31, 0.46, 0.69);
  B(A.wall, 0.46, 0.44, 0.5, 0.32, 0.28, 0.69); EAVE(A.roof, 0.6, 0.12, 0.024, 0.045, 0.54, 0.32, 0.5, 0.69);
  B(A.plaza, 0.68, 0.066, 0.74, 0.94, 0.033, 0.55);
  function lamp(x, z) { Y(A.steel, 0.01, 0.013, 0.5, 6, x, 0.31, z); B(A.steel, 0.12, 0.014, 0.014, x - 0.05, 0.555, z); B(lampM || A.steel, 0.07, 0.02, 0.045, x - 0.1, 0.545, z); }
  function bench(x, z) { B(A.wood, 0.24, 0.025, 0.08, x, 0.15, z); B(A.wood, 0.02, 0.07, 0.07, x - 0.1, 0.1, z); B(A.wood, 0.02, 0.07, 0.07, x + 0.1, 0.1, z); }
  lamp(1.22, 0.86); bench(1.0, 0.82);
  if (level === 1) {
    B(A.wood, 0.24, 0.28, 0.02, -0.44, 0.2, 0.945); B(A.wood, 0.24, 0.28, 0.02, -0.18, 0.2, 0.945);
    B(A.wood, 0.22, 0.28, 0.02, 0.22, 0.2, 0.945); B(A.win, 0.14, 0.16, 0.02, 0.44, 0.26, 0.945);
  } else {
    B(glow, 0.3, 0.26, 0.02, -0.42, 0.21, 0.945); B(A.wood, 0.2, 0.26, 0.02, -0.14, 0.21, 0.945);
    B(glow, 0.26, 0.26, 0.02, 0.26, 0.21, 0.945); B(level >= 3 ? glow : A.win, 0.14, 0.16, 0.02, 0.48, 0.26, 0.945);
    lamp(0.68, 0.9); bench(0.72, 0.62); B(A.white, 0.16, 0.08, 0.12, 0.66, 0.1, 0.26); O(A.leaf2, 0.055, 0.66, 0.19, 0.26);
  }
  if (level >= 3) { /* 布艺遮阳篷 + 广场绿篱 + 巷口街灯 */
    B(A.awn, 0.6, 0.016, 0.22, -0.31, 0.52, 1.0, -0.45, 0, 0); B(A.awn, 0.6, 0.05, 0.02, -0.31, 0.47, 1.1);
    B(A.awn, 0.42, 0.016, 0.22, 0.32, 0.56, 1.0, -0.45, 0, 0); B(A.awn, 0.42, 0.05, 0.02, 0.32, 0.51, 1.1);
    B(A.leaf2, 0.62, 0.07, 0.05, 0.95, 0.1, 0.92); B(A.leaf2, 0.05, 0.07, 0.7, 1.27, 0.1, 0.55); lamp(0.05, 0.4);
  }
  /* 巷门头：灰砖门柱 + 木枋 + 卷檐匾额，lv3+ 双层翘檐 */
  B(A.wall, 0.11, 0.62, 0.11, -1.17, 0.37, 0.69); B(A.wall, 0.11, 0.62, 0.11, -0.71, 0.37, 0.69);
  B(A.wood, 0.64, 0.07, 0.13, -0.94, 0.715, 0.69);
  if (level >= 2) B(signM, 0.36, 0.11, 0.028, -0.94, 0.715, 0.762);
  EAVE(A.roof, 0.68, 0.13, 0.024, 0.05, 0.62, -0.94, 0.75, 0.69);
  lan(-1.13, 0.66, 0.79, 0.9, 1); if (level >= 2) lan(-0.75, 0.66, 0.79, 0.9, 1);
  if (level >= 3) {
    B(A.wood, 0.04, 0.1, 0.04, -1.1, 0.95, 0.6); B(A.wood, 0.04, 0.1, 0.04, -0.78, 0.95, 0.6); B(A.wood, 0.04, 0.1, 0.04, -1.1, 0.95, 0.78); B(A.wood, 0.04, 0.1, 0.04, -0.78, 0.95, 0.78);
    EAVE(A.roof, 0.5, 0.11, 0.02, 0.045, 0.44, -0.94, 1.0, 0.69);
    Y(A.gold, 0.014, 0.02, 0.05, 6, -0.94, 1.155, 0.69);
  }
  /* 灯笼串跨巷（lv2+，lv4 满巷点亮 + 门灯 + 花箱） */
  if (level >= 2) {
    B(A.wire, 1.7, 0.006, 0.006, 0.25, 0.68, 0.05);
    lan(-0.2, 0.62, 0.05); lan(0.25, 0.62, 0.05); lan(0.7, 0.62, 0.05);
    if (level >= 3) { B(A.wire, 1.4, 0.006, 0.006, 0.3, 0.68, 0.26); lan(0.0, 0.62, 0.26); lan(0.45, 0.62, 0.26); lan(0.9, 0.62, 0.26); }
    if (level >= 4) {
      B(A.wire, 1.2, 0.006, 0.006, 0.45, 0.68, -0.08); lan(0.1, 0.62, -0.08); lan(0.55, 0.62, -0.08); lan(1.0, 0.62, -0.08);
      lan(-0.42, 0.44, 1.0, 0.85, 1); lan(0.26, 0.44, 1.0, 0.85, 1);
      B(A.white, 0.18, 0.09, 0.13, -0.05, 0.105, 0.44); O(A.leaf2, 0.06, -0.05, 0.2, 0.44);
      B(A.white, 0.18, 0.09, 0.13, 1.2, 0.105, 0.36); O(A.leaf2, 0.06, 1.2, 0.2, 0.36); bench(1.14, 0.5);
    }
  }
  /* 茶馆白伞座（lv2+ 巷内，lv3+ 广场） */
  if (level >= 2) {
    function um(x, z) { Y(A.wood, 0.007, 0.007, 0.36, 5, x, 0.24, z); Y(A.umb, 0.012, 0.15, 0.1, 8, x, 0.46, z); Y(A.wood, 0.05, 0.05, 0.04, 6, x + 0.17, 0.084, z + 0.13); }
    um(-0.58, 0.12); um(0.18, 0.04); if (level >= 3) um(0.86, 0.72);
  }
  /* 屋面加建：lv2 小披屋 → lv3 白色凉亭 → lv4 玻璃阳光房+钢架+天线 */
  if (level >= 2) { B(A.white, 0.4, 0.18, 0.34, -0.9, 0.83, -0.56); B(A.roof, 0.46, 0.02, 0.4, -0.9, 0.93, -0.56); }
  if (level >= 3) { B(A.white, 0.44, 0.2, 0.36, 0.32, 0.93, -0.56); B(A.roof, 0.5, 0.02, 0.42, 0.32, 1.04, -0.56); }
  if (level >= 4) {
    B(A.sky, 0.4, 0.18, 0.32, -0.9, 1.02, -0.56); B(A.steel, 0.44, 0.015, 0.36, -0.9, 1.12, -0.56);
    Y(A.steel, 0.006, 0.006, 0.14, 5, -0.76, 1.19, -0.62);
  }
  /* 巷弄/广场乔木：逐年代增多长高（冠幅 0.15→0.23） */
  var TP = [[[-0.42, -0.04], [0.6, 0.0], [1.02, 0.62]], [[-0.42, -0.04], [0.6, 0.0], [1.02, 0.62], [-1.08, 0.1], [0.8, 0.36]],
    [[-0.42, -0.04], [0.58, 0.02], [1.02, 0.62], [-1.08, 0.1], [0.8, 0.36], [-0.24, 0.28], [1.06, 0.2]], [[-0.4, -0.02], [0.56, 0.04], [1.02, 0.62], [-1.04, 0.12], [0.78, 0.36], [-0.22, 0.3], [1.02, 0.18], [0.2, -0.1]]][level - 1];
  var cr = [0.15, 0.18, 0.2, 0.23][level - 1];
